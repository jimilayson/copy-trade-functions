...
// Temporary type fix for Firebase Functions v2
declare module 'firebase-functions' {
  namespace pubsub {
    function schedule(cron: string): {
      timeZone: (tz: string) => {
        onRun: (handler: (context: any) => Promise<any>) => any;
      };
    };
  }
}
...
export const createTrader = functions.https.onCall(async (data: any, context: any) => {
  // بررسی احراز هویت
  if (!context.auth) {
...
export const updateTraderStats = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const incrementFollowerCount = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const decrementFollowerCount = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const createBot = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const createTrade = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
  ...
  try {
    const {
      symbol,
      side, // استفاده از side به جای direction
      entryPrice,
      quantity,
      stopLoss,
      takeProfit
    ...
    const tradeData = {
      ...
      symbol,
      direction: side, // نگاشت side به direction برای سازگاری
      entryPrice: parseFloat(entryPrice),
      quantity: parseFloat(quantity),
      exitPrice: 0, // مقدار اولیه
      stopLoss: stopLoss ? parseFloat(stopLoss) : null,
      takeProfit: takeProfit ? parseFloat(takeProfit) : null,
      status: 'open',
      profitLoss: 0, // معادل pnl
      pnlPercentage: 0, // فیلد جدید
      copiedByCount: 0, // فیلد جدید
...
// تابع برای بستن معامله - نسخه اصلاح شده با به‌روزرسانی موجودی
export const closeTrade = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const getUserTrades = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
  ...
  try {
    ...
    const trades = snapshot.docs.map(doc => {
      ...
      return {
        ...
        symbol: data.symbol,
        side: data.direction, // تبدیل direction به side
        entryPrice: data.entryPrice,
        quantity: data.quantity,
        stopLoss: data.stopLoss,
        takeProfit: data.takeProfit,
        status: data.status.toUpperCase() as 'OPEN' | 'CLOSED', // تبدیل به uppercase
...
export const getAllTraders = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const copyTrade = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
  ...
  try {
    ...
    const copyTradeData = {
      ...
      quantity: parseFloat(amount) / originalTrade.entryPrice, // محاسبه مقدار بر اساس سرمایه
...
// 🔽 توابع جدید برای Price Feed و Trade Watcher 🔽

// اینترفیس برای معامله باز
interface OpenTrade {
  id: string;
  traderId: string;
  symbol: string;
...
export const startPriceFeed = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const stopPriceFeed = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
...
export const checkOpenTrades = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
  ...
  try {
    ...
    const currentPrice = priceDoc.data()?.price;
    if (!currentPrice) {
    ...
    for (const tradeDoc of openTradesSnapshot.docs) {
      const trade = tradeDoc.data() as OpenTrade;
      
      const shouldClose = 
        (trade.side === 'BUY' && trade.stopLoss && currentPrice <= trade.stopLoss) || // SL برای BUY
...
export const internalTransfer = functions.https.onCall(async (request) => {
...
// 🔽 تابع جدید برای محاسبه خودکار امتیاز معامله‌گران با فرمول v2.0 🔽

// اینترفیس برای داده‌های محاسبه امتیاز v2.0
interface TraderScoreDataV2 {
  totalReturn: number;
...
function calculateTraderScoreV2(data: TraderScoreDataV2): { score: number; overallStatus: string } {
  ...
  try {
    ...
    const weightedScore = 
      (R1 * 0.30) +   // بازدهی کل 30% (افزایش یافت)
...
export const calculateInternalTraderScore = functions.firestore
  .onDocumentUpdated('internalTraders/{traderId}', async (event) => {
    try {
      ...
      const scoreData: TraderScoreDataV2 = {
        totalReturn: calculatedData.totalReturn,
        monthlyReturn: calculatedData.monthlyReturn, // استفاده از مقدار محاسبه شده REAL
        winRate: calculatedData.winRate,
        maxDrawdown: calculatedData.maxDrawdown, // استفاده از مقدار محاسبه شده REAL
        followersCount: afterData?.followersCount || afterData?.followers || 0,
      ...
      await event.data.after.ref.update({
        score,
        overallStatus,
        monthlyReturn: calculatedData.monthlyReturn, // ذخیره مقدار REAL
        maxDrawdown: calculatedData.maxDrawdown, // ذخیره مقدار REAL
        totalReturn: calculatedData.totalReturn,
        winRate: calculatedData.winRate,
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      });
...
async function calculateRealTraderMetrics(traderData: any, traderId: string) {
...
export const manualCalculateScore = functions.https.onCall(async (request) => {
  const { data, auth } = request;
...
export const manualUpdateDailyActivity = functions.https.onRequest(async (request, response) => {
  try {
...
export const manualDailyUpdate = functions.https.onRequest(async (req, res) => {
  try {
...
// تابع اصلی بروزرسانی
async function updateAllTradersDaily(): Promise<void> {
  try {
    console.log('🔄 شروع بروزرسانی خودکار روزانه...');
    
    // بروزرسانی معامله‌گران داخلی
    const internalTraders = await admin.firestore().collection('internalTraders').get();
    const internalUpdates = internalTraders.docs.map(doc => 
      doc.ref.update({
        daysActive: admin.firestore.FieldValue.increment(1),
        lastActivityUpdate: admin.firestore.FieldValue.serverTimestamp()
      })
    ...
    // بروزرسانی ربات‌ها
    const bots = await admin.firestore().collection('bots').get();
    const botUpdates = bots.docs.map(doc =>
      doc.ref.update({
        daysActive: admin.firestore.FieldValue.increment(1), 
        lastActivityUpdate: admin.firestore.FieldValue.serverTimestamp()
      })
...
async function checkMonthlyUpdate(): Promise<void> {
  ...
  if (today.getDate() !== 1) return; // فقط اول ماه
...
export const recalculateActivityDuration = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'لطفاً وارد شوید');
  }
